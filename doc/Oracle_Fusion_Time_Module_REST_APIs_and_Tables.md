# Oracle Fusion REST APIs & Tables for a Custom Time Module (OTL Replacement)

> Compiled from Oracle Fusion Cloud documentation (release 26C): REST API books (HCM `farws`, Project Management `fapap`) and Tables & Views books (HCM `oedmh`, Project Management `oedpp`).
> **REST base paths:** HCM = `/hcmRestApi/resources/11.13.18.05/` · Projects/PPM = `/fscmRestApi/resources/11.13.18.05/`
> Auth: Basic Auth or OAuth 2.0 over HTTPS. Includes endpoints, example request/response bodies, field-level schemas, **and the physical database tables** where each type of data is stored.

## Architecture Overview

| Requirement | Nature | REST Mechanism | Key Tables |
|---|---|---|---|
| 1. POET | REST (reference) | `projects`, task, `expenditureTypes` | PJF_PROJECTS_ALL_B, PJF_PROJ_ELEMENTS_B, PJF_EXP_TYPES_B |
| 2. Project resources | REST | `projectEnterpriseResources`, `projectLaborResources` | PJF_PROJECT_PARTIES, PJT_PROJECT_RESOURCE |
| 3. Project task | REST | `projects/{id}/child/Tasks` | PJF_PROJ_ELEMENTS_B, PJF_TASKS_V |
| 4. Project allocation | REST | `projectResourceAssignments`, `personAssignmentLaborSchedules` | PJR_ASSIGNMENT, HWM_ALLOCATIONS_HDR_F |
| 5. Resource cost | REST | `projectCosts`, `projectExpenditureItems`, `rateSchedules` | PJC_EXP_ITEMS_ALL, PJC_COST_DIST_LINES_ALL |
| 6. Time → Payroll | REST + ESS | `timeRecordEventRequests` → `elementEntries` | HWM_TM_REC, PAY_ELEMENT_ENTRIES_F |
| 7. Approved time → Costing | ESS process | Import & Process Cost Transactions | PJC_TXN_XFACE_ALL, PJC_EXP_ITEMS_ALL |
| 8. Shift / pattern / schedule | Config + REST | `workforceScheduleDefinitions` | HTS_WORK_PATTERNS_B, PER_SCHEDULE_ASSIGNMENTS |
| 9. Calendar & holidays | Config (HDL) | Calendar Events | PER_CALENDAR_EVENTS |
| 10. Default absence | REST | `absences`, `planBalances` | ANC_PER_ABS_ENTRIES |

---

## 1. POET Information

POET = **P**roject, **O**rganization, **E**xpenditure type, **T**ask. Assembled from several resources.
...
> _Showing 55 of 421 fields (core attributes). See Oracle docs for the full list._
...
> **Task ("T")** = Project Tasks (Section 3). **Organization ("O")** = project's owning/expenditure org + person's assignment.
...
#### Project Labor Resources – add to project
...
> _Showing 35 of 37 fields (core attributes). See Oracle docs for the full list._

---

## 3. Project Task

- **Tasks of a project:** `GET /fscmRestApi/resources/11.13.18.05/projects/{ProjectId}/child/Tasks`
- **Work-plan tasks:** `GET .../projectPlans/{ProjectId}/child/Tasks`
- **Financial tasks:** `GET .../projectFinancialTasks`

#### Project Plans (task structure)
...
> _Showing 45 of 63 fields (core attributes). See Oracle docs for the full list._
#### Person Assignment Labor Schedules – create (percentage allocation by POET)
...
> _Showing 45 of 97 fields (core attributes). See Oracle docs for the full list._
...
> _Showing 55 of 207 fields (core attributes). See Oracle docs for the full list._
...
> `projectCosts` has GET/PATCH + `adjustProjectCosts` action, **no direct POST** — costs are created by the *Import and Process Cost Transactions* process.
...
**Step A — Capture (REST):** `timeRecordEventRequests` with `processMode: "TIME_SUBMIT"`. Payroll/project attributes (e.g. `PayrollTimeType`, POET) passed as `timeRecordEventAttribute` name/value pairs.
...
#### Time Records – read posted record
...
**Step B — Transfer to Payroll (ESS process):** *Load Time Card Batches / Transfer Time Cards from Time and Labor* moves approved hours into Payroll **Element Entries**.
...
Process-driven: **Transfer Time to Projects** → **Import and Process Cost Transactions**. Results in `projectCosts` / `projectExpenditureItems` (Section 5). REST staging alternative: **Project Expenditure Batches** (`GET/PATCH .../projectExpenditureBatches`) then run the import; adjustments via `POST .../projectCosts/{id}/action/adjustProjectCosts`.

---

## 8. Employee Shift, Work Pattern & Schedule

Primarily setup/config (HDL "Work Schedule" / "Work Schedule Assignment"). REST: `workforceScheduleDefinitions`, `workforceScheduleShifts`, `scheduleRequests`, `staffingGrids`, `schedulingShiftsLOV`; Time & Labor: `timeLayoutSets`, `webClockEvents`, `geofences`.

---

## 9. Calendar & Holidays

Holiday calendars = **Calendar Events / schedule exceptions** (config via HDL). Non-working days derived from the assigned Work Schedule Definition + exceptions.
...
> Supporting LOVs: `absenceTypesLOV`, `absenceTypeReasonsLOV`, `absencePlansLOV`. `planBalances` for available balance; `absencesNoEntitlements` for absences without entitlement processing.


---

## Physical Database Tables & Views
...
## Quick REST Endpoint Index

| # | Purpose | Method | Resource path |
|---|---|---|---|
| 1 | Projects | GET/POST/PATCH | `/fscmRestApi/.../projects` |
| 1 | Expenditure Types | GET | `/fscmRestApi/.../expenditureTypes` |
...
---
*Generated from Oracle Fusion Cloud Applications documentation (release 26C). REST field schemas trimmed to core attributes for readability. Table names verified against Oracle's Tables & Views books; `_ALL` tables are multi-org and typically queried via their secured `_VL`/`_V` views. Verify complete column lists, flexfields, and privileges against your pod before build.*
