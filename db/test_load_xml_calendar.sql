...
PROMPT
PROMPT === the REAL envelope, not just the real rows =================
PROMPT The block above used rows copied from live output, but hand-wrapped.
PROMPT Inspecting what BIP actually returns shows three things a hand-written
PROMPT sample leaves out, and only one of them is obviously harmless:
PROMPT
PROMPT   1. an <?xml ...?> declaration
PROMPT   2. an Oracle-generated <!-- comment --> after it
PROMPT   3. the PARAMETERS echoed as elements directly under DATA_DS,
PROMPT      BEFORE <ROWSET>, because the model sets include_parameters=true
PROMPT
PROMPT (3) is the one worth proving. P_EFFECTIVE_DATE and P_LAST_SYNC are
PROMPT siblings of ROWSET, so /DATA_DS/ROWSET/ROW should step straight past
PROMPT them -- but if the XPath were ever loosened to //ROW or the column
PROMPT scan to /DATA_DS/*, they would be read as data. Verified, not assumed.
PROMPT
PROMPT Confirmed absent from the live payload: gzip, and any namespace on
PROMPT DATA_DS. A namespace would make /DATA_DS/ROWSET/ROW match nothing and
PROMPT return zero rows while reporting success.
PROMPT ==============================================================
...
BEGIN
  -- Byte-for-byte the shape of a real O2C_CALENDAR.xdm response.
  v_xml :=
'<?xml version="1.0" encoding="UTF-8"?>
<!--Generated by Oracle Analytics Publisher -Dataengine, datamodel:_Custom_O2C_TIME_O2C_CALENDAR_xdm -->
<DATA_DS><P_EFFECTIVE_DATE>2026-08-01</P_EFFECTIVE_DATE><P_LAST_SYNC>1900-01-01</P_LAST_SYNC>
<ROWSET>
<ROW><LAYER>CORPORATE</LAYER><SCOPE_KEY>ENVTEST1</SCOPE_KEY><CAL_DATE>2026-05-04</CAL_DATE><IS_WORKING_DAY>N</IS_WORKING_DAY><HOLIDAY_NAME>Envelope test</HOLIDAY_NAME><SHIFT_CODE></SHIFT_CODE><STD_HOURS>0</STD_HOURS></ROW>
<ROW><LAYER>CORPORATE</LAYER><SCOPE_KEY>ENVTEST2</SCOPE_KEY><CAL_DATE>2026-05-05</CAL_DATE><IS_WORKING_DAY>N</IS_WORKING_DAY><HOLIDAY_NAME>Envelope test</HOLIDAY_NAME><SHIFT_CODE></SHIFT_CODE><STD_HOURS>0</STD_HOURS></ROW>
</ROWSET></DATA_DS>';

  oc_time_load_xml('OC_TIME_CALENDAR', v_xml, 'CALENDAR', 'ENVELOPE_TEST',
                   v_read, v_merged, v_status, v_msg);

  DBMS_OUTPUT.PUT_LINE('status : ' || v_status);
  DBMS_OUTPUT.PUT_LINE('read   : ' || v_read || '   (must be 2, NOT 4 --');
  DBMS_OUTPUT.PUT_LINE('         4 would mean the two echoed parameters were');
  DBMS_OUTPUT.PUT_LINE('         counted as data rows)');
  DBMS_OUTPUT.PUT_LINE('message: ' || v_msg);

  -- And the parameters must not have leaked into a column.
  SELECT COUNT(*) INTO v_n FROM oc_time_calendar
   WHERE scope_key IN ('ENVTEST1','ENVTEST2')
     AND holiday_name = 'Envelope test';
  DBMS_OUTPUT.PUT_LINE('clean rows: ' || v_n || '   (must be 2)');

  IF v_status = 'Success' AND v_read = 2 AND v_n = 2 THEN
    DBMS_OUTPUT.PUT_LINE('PASS - declaration, comment and echoed parameters');
    DBMS_OUTPUT.PUT_LINE('       are all stepped over correctly.');
  ELSE
    DBMS_OUTPUT.PUT_LINE('FAIL - the real envelope is not being handled.');
  END IF;
END;
/

DELETE FROM oc_time_calendar WHERE scope_key IN ('ENVTEST1','ENVTEST2');
COMMIT;
